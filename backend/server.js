...
function calcDashboard(wb) {
  ...
  aRows.forEach(r => {
    ...
    // Install date → actual (ถ้าไม่มี Install Date แต่ ok>0 ให้ใช้ Scheduled Date แทน)
    let instDt = r['Install Date'];
    if (!instDt && ok > 0) instDt = new Date();  // ติดตั้งแล้วแต่ไม่มีวันที่ → ใช้วันนี้
...
app.get('/api/devices', async (req, res) => {
  try {
    ...
    res.json({ data: (d.hold_items || []).slice(0, parseInt(limit)) });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
